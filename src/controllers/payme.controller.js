...
const marketplaceRepo = require('../marketplace-repository.js');
...
const resErr = (id, code, message, data) => ({
  jsonrpc: '2.0',
...
function parseAuthorization(header = '') {
  ...
  try {
    const [username, password] = Buffer.from(String(header).slice(6), 'base64')
      .toString('utf8')
      .split(':');
...
function configReady() {
  const cfg = getConfig();
  return Boolean(cfg.secret || (cfg.testMode && cfg.legacyTestKey));
}
...
async function findOrder(account = {}) {
  ...
  if (cfg.testMode && /^test(-\d+)?$/i.test(orderKey)) {
    const tiyin = /^test-(\d+)$/i.test(orderKey)
      ? Number(orderKey.split('-')[1]) * 10000
      : 5000;
    return {
      id: orderKey,
  ...
  const row = await marketplaceRepo.findOrderForPayme(orderKey);
  if (!row) return null;
  return {
    id: row.id,
...
async function paymeRpc(req, res) {
  ...
  if (!isAuthorized(req)) {
    return send(res, resErr(id, ERRORS.UNAUTHORIZED, msg('Unauthorized', 'Не авторизован', 'Avtorizatsiyadan o‘tilmagan'), 'authorization'));
  }
  if (!configReady()) {
    return send(
      res,
      resErr(id, ERRORS.CONFIG, msg('Payment config error', 'Ошибка конфигурации оплаты', 'To‘lov konfiguratsiyasi xatosi'), 'payme_config')
    );
  }
  try {
    if (method === 'CheckPerformTransaction') {
      ...
      if (!order) {
        return send(res, resErr(id, ERRORS.ORDER_NOT_FOUND, msg('Order not found', 'Заказ не найден', 'Buyurtma topilmadi'), 'order_id'));
      }
      if (Number(params.amount) !== expectedAmountTiyin(order)) {
        return send(res, resErr(id, ERRORS.INVALID_AMOUNT, msg('Incorrect amount', 'Неверная сумма', 'Noto‘g‘ri summa'), 'amount'));
      }
    ...
    if (method === 'CreateTransaction') {
      ...
      if (existing) {
        return send(
          res,
          resOk(id, {
            create_time: Number(existing.create_time || 0),
      ...
      if (active) {
        return send(
          res,
          resErr(id, ERRORS.TX_EXISTS, msg('Transaction already exists', 'Транзакция уже существует', 'Tranzaksiya allaqachon mavjud'), 'order_id')
        );
      }
      const tx = await txRepo.upsert({
    ...
    if (method === 'PerformTransaction') {
      ...
      if (!tx) {
        return send(res, resErr(id, ERRORS.TX_NOT_FOUND, msg('Transaction not found', 'Транзакция не найдена', 'Tranzaksiya topilmadi'), 'id'));
      }
      if (Number(tx.state) < 0) {
        return send(
          res,
          resErr(id, ERRORS.ORDER_NOT_FOUND, msg('Transaction cancelled', 'Транзакция отменена', 'Tranzaksiya bekor qilingan'), 'id')
        );
      }
      if (Number(tx.state) !== TX_STATE.PERFORMED) {
        ...
        if (!tx.sandbox) await marketplaceRepo.paymeMarkOrderPaid(tx.order_id);
      }
      return send(
        res,
        resOk(id, {
    ...
    if (method === 'CancelTransaction') {
      ...
      if (![TX_STATE.CANCELED_BEFORE_PERFORM, TX_STATE.CANCELED_AFTER_PERFORM].includes(Number(tx.state))) {
        ...
        const prev = Number(tx.state);
        tx.state =
          prev === TX_STATE.PERFORMED ? TX_STATE.CANCELED_AFTER_PERFORM : TX_STATE.CANCELED_BEFORE_PERFORM;
        await txRepo.upsert(tx);
        if (!tx.sandbox && tx.state === TX_STATE.CANCELED_BEFORE_PERFORM) {
          await marketplaceRepo.paymeMarkOrderPaymentPending(tx.order_id);
        }
      }
      return send(
        res,
