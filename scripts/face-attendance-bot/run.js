/**
 * @sklad_nazoratchi_bot — Face ID → Telegram (faqat tanlangan xodimlar)
 * npm run face-bot
 */
...
function formatCheckInMessage(ev) {
  ...
  const kind = labelMinor(ev.minor);
  return `📥 Keldi\n👤 ${name}\n🕐 ${formatClock(at)}\n📋 ${kind}`;
}

function formatCheckOutMessage(row, durationText) {
  const inD = new Date(row.checkIn);
  const outD = new Date(row.checkOut);
  return (
    `📤 Ketdi\n👤 ${row.name}\n📥 ${formatClock(inD)} → 📤 ${formatClock(outD)}\n⏱ Ish vaqti: ${durationText}`
  );
}
...
async function processDeviceEvents(config, state, dateKey) {
  ...
  for (const ev of events) {
    ...
    if (result.type === 'out' && result.durationText) {
      await sendMessage(
        config.token,
        config.chatId,
...
async function main() {
  ...
  console.log(
    `[sklad-bot] Face: http://${config.ip} | kuzatilgan: ${watchlistCount(state)} | poll ${config.pollSec}s`
  );

  const tick = async () => {
    try {
      const { total, sent, skipped } = await processDeviceEvents(config, state, dateKey);
      if (sent || skipped) {
        console.log(`[face] hodisa ${total}, xabar ${sent}, o'tkazildi ${skipped}`);
      }
    } catch (e) {
