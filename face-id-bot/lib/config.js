...
function cfg() {
  ...
  const chatId = String(process.env.FACE_NOTIFY_CHAT_ID || '').trim();
  ...
  if (!password) throw new Error('FACE_DEVICE_PASSWORD .env da kerak');
  if (!token) throw new Error('AI_DAVLAT_BOT_TOKEN (@ai_davlat_agent_pro_bot) kerak');
  if (!chatId) throw new Error('FACE_NOTIFY_CHAT_ID kerak');
