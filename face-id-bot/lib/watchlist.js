...
function isWatched(state, ev) {
  ...
  for (const [, row] of Object.entries(list)) {
...
function isWatchedEmpKey(state, empKey) {
  return Boolean(ensureWatchlist(state)[empKey]);
}

function listWatched(state) {
  return Object.entries(ensureWatchlist(state)).map(([key, row]) => ({
    key,
    name: row.name || key
  }));
}
...
function removeWatch(state, query, dateKey) {
  const list = ensureWatchlist(state);
  const q = norm(query);
  if (!q) return { ok: false, message: 'Ism yozing: /olib Ali' };
  ...
  if (match?.ambiguous) {
    return {
      ok: false,
      message: `Bir nechta: ${match.ambiguous.map((x) => x.name).join(', ')}`
    };
  }
  if (match?.key) {
  ...
  return { ok: false, message: 'Roʻyxatda topilmadi.' };
}
...
function addWatchByQuery(state, dateKey, query) {
  ...
  if (match?.ambiguous) {
    return {
      ok: false,
      message: `Bir nechta: ${match.ambiguous.map((x) => x.name).join(', ')}. /kuzat dan tanlang.`
    };
  }
  if (!match?.key) {
    return {
      ok: false,
      message: "Bugun terminalda topilmadi. Avval Face ID dan o'tkazing yoki /hodimlar."
    };
  }
  addWatch(state, match.key, match.name);
  return { ok: true, message: `✅ Kuzatiladi: ${match.name}` };
}
